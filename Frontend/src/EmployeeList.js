...
const EmployeeList = () => {
    ...
    useEffect(() => {
        const fetchEmployees = async () => {
            try {
                ...
                const response = await axios.get('http://localhost:5001/api/employees/all', config);
    ...
    const handleDeleteEmployee = async (id) => {
        ...
        try {
            ...
            const response = await axios.delete(`http://localhost:5001/api/employees/delete/${id}`,config);
            ...
            const responselist = await axios.get('http://localhost:5001/api/employees/all', configlist);
    ...
    const handleSubmit = async (values, { setSubmitting }) => {
        ...
        try {
            if (currentEmployee) {
                ...
                // Edit employee
                await axios.put(`http://localhost:5001/api/employees/update/${currentEmployee.id}`, employeeData, config);
            } else {
                // Create employee
                const token = localStorage.getItem('auth-token');
                const config = {
                ...
                await axios.post('http://localhost:5001/api/employees/create', employeeData, config);
            }
            setOpenModal(false);
            setCurrentEmployee(null);
            setImage(null); // Reset image after submission
    ...
    return (
        <div style={{ padding: '20px', backgroundColor: '#f5f5f5' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px', alignItems: 'center' }}>
                <span style={{ fontWeight: 'bold',color:'black' }}>Total Count: {filteredEmployees.length}</span>
                        ...
                        {headerGroups.map(headerGroup => (
                            <TableRow {...headerGroup.getHeaderGroupProps()}>
                                {headerGroup.headers.map(column => (
                                    <TableCell {...column.getHeaderProps()} style={{ backgroundColor: '#491441', color: '#fff' }}>{column.render('Header')}</TableCell>
                                ))}
                            </TableRow>
                        ))}
                    </TableHead>
