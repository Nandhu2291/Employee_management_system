...
const Middleware = require('../middleware/authMiddleware'); // Import the auth middleware
...
router.post('/create', upload.single('f_Image'), async (req, res) => {
    ...
    try {
        ...
        const Mob_num_exist = await Employee.findOne({ where: { f_Mobile }});
        if (Mob_num_exist) return res.status(400).send('Mobile number already exists')
